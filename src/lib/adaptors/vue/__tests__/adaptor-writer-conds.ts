...
const writeConditionsWrong: WriteCondition[] = [
  {
    description: "doesn't do anything on empty array",
    fileChanges: [
  ...
  },
  {
    ...
    fileChanges: [
      [relPath('./fixtures/wrong.txt'), relPath('./fixtures/wrong-added.txt')],
    ],
    data: [
      new DefinitionItem({
        locale: 'bz',
...
const writeConditionsNonexist: WriteCondition[] = [
  ...
  {
    description: "throws when source doesn't exist",
    data: [
...
const writeConditionsPopulated: WriteCondition[] = [
  ...
  },
  {
    ...
    fileChanges: [
      [relPath('./fixtures/write.vue'), relPath('./fixtures/write-subset.vue')],
    ],
    data: [...joinDefinition([enDefinitions], relPath('./fixtures/write.vue'))],
    options: {
      contentFormatter: new ContentFormatter({
        afterBlock: '',
        afterGroup: '',
...
const writeConditionsLinked: WriteCondition[] = [
  ...
  },
  {
    ...
    meta: {
      ...
      linkedShouldExist: [
        [
          relPath('./fixtures/write-empty.vue'),
          [['./write-linked2.json', relPath('./fixtures/write-linked2.json')]],
        ],
      ],
    },
  },
  ...
  {
    description: 'removes linked file if no items should be written to it',
    fileChanges: [
      [
        relPath('./fixtures/write-linked.vue'),
...
export const conditionGroups: {
  ...
  { description: 'linked', conditions: writeConditionsLinked },
